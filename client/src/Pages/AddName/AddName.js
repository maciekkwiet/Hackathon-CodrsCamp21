...
const onSubmitHandler = (  ) => {
  ...
  <Grid.Column style={{ width: 450 }}>
    ...
    <Form size='large' onSubmit={()=>onSubmitHandler()}>
      <Segment stacked style={{ width: 450, height:450, display:'flex', flexDirection:'column', alignItem: 'center', justifyContent: 'center'}}>
      <Image src={meeting} size='medium' rounded style={{marginBottom:40, marginLeft:50}}/>
        <Form.Input  fluid icon='user' iconPosition='left' placeholder='Imię' onChange={(e)=>{setNameUser(e.target.value)}} />
        <Button color='teal' type='submit' fluid size='large' onClick={()=>{onSubmitHandler()}}>
         Dołączam
        </Button>
      </Segment>
    </Form>
